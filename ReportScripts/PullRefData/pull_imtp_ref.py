...
from config import GCP_CREDENTIALS_PATH, GCP_PROJECT_ID, IMTP_TABLE

key_path = GCP_CREDENTIALS_PATH
project_id = GCP_PROJECT_ID
_TABLE_FQN = IMTP_TABLE
...
def pull_imtp_ref(min_age: int, max_age: int) -> pd.DataFrame:
    ...
    # Keeping only the best "ISO_BM_REL_FORCE_PEAK_Trial_N_kg" value for each athlete
    df = df.sort_values(by="ISO_BM_REL_FORCE_PEAK_Trial_N_kg", ascending=False)
